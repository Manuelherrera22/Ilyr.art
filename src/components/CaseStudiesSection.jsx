...
const CaseStudiesSection = () => {
  ...
  return (
    <section id="case-studies" className="py-28 bg-gradient-to-b from-background via-card/30 to-background section-intro-glow">
          ...
          {caseStudies.map((study, index) => (
            ...
            >
              <Card className={`glass-card rounded-2xl overflow-hidden shadow-2xl shadow-${study.themeColor}/10 h-full flex flex-col border-t-2 border-${study.themeColor}/50 perspective-element focus-within:ring-2 focus-within:ring-accent focus-within:ring-offset-2`}>
                <div className={`relative h-72 overflow-hidden`} role="img" aria-label={study.altText}>
                  <img
                  ...
                  <div className={`absolute top-4 right-4 bg-${study.themeColor}/80 backdrop-blur-sm text-background px-3 py-1 rounded-lg text-xs font-bold shadow-lg`} aria-label="Potenciado por IA">
                    POTENCIADO POR IA
                  </div>
                   <div className="absolute bottom-4 left-4 flex gap-2" role="list" aria-label="Etiquetas del proyecto">
                    {study.tags.map(tag => (
                       <span key={tag} className={`text-xs px-2.5 py-1 rounded-full bg-background/70 backdrop-blur-sm text-${study.themeColor} border border-${study.themeColor}/30`} role="listitem">{tag}</span>
                    ))}
                  </div>
                </div>

                <CardHeader className="p-8 pb-0">
                   <CardTitle className={`text-3xl font-bold text-foreground mb-1 group-hover:text-${study.themeColor} transition-colors`}>
                    {study.title}
                  </CardTitle>
                </CardHeader>

                <CardContent className="p-8 flex-grow flex flex-col">
                  <p className="text-secondary-type text-sm mb-6 leading-relaxed flex-grow">
                    {study.description}
                  </p>

                  <div className={`bg-card/50 rounded-xl p-6 border border-${study.themeColor}/20 shadow-inner shadow-${study.themeColor}/5 mt-auto`}>
                    <Quote className={`w-8 h-8 text-${study.themeColor} mb-4 -ml-1`} />
                    <p className="text-foreground/90 text-sm italic mb-5 leading-relaxed">
                      "{study.testimonial}"
                    </p>
